import { documentMouseUp } from 'newUtils'
...
export default class Carousel extends React.Component {
  ...
  scrollClick(path) {
    let nextScroll = this.props.scrollLength * (path ? -1 : 1)
  ...
  mouseMove(e) {
    if (this.state.resolution) {
      let indentY = (this.props.direction != 'vertical' ? e.clientX : e.clientY) - this.state.defaultY 
    ...
    return (
      <div 
        ...
        </div>
        {
          this.props.arrows ?
          <div>
            <button className="carousel__arrow carousel__arrow_direction_prev" type="button" onMouseDown={ this.scrollClick.bind(this, true) }></button>
            <button className="carousel__arrow carousel__arrow_direction_next" type="button" onMouseDown={ this.scrollClick.bind(this, false) }></button>
          </div>
          : null
        }
      </div>
